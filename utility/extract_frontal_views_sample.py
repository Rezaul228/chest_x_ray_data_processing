...
def extract_frontal_views_sample():
    """Extract frontal views from ReXGradient dataset - Sample of 2000 studies"""
    ...
    print("="*80)
    print("Frontal View Extraction - ReXGradient Dataset (2000 Sample)")
    print("="*80)
    
    # Get all metadata files
    metadata_files = [f for f in os.listdir(metadata_dir) if f.endswith('.json')]
    print(f"Total metadata files available: {len(metadata_files)}")
    ...
    # Expanded frontal view study types based on actual data
    frontal_study_types = [
        ...
        'XR chest 2V',           # X-ray two-view
        'DG CHEST PORT W/ABD NEONATE', # Portable chest
        'DG CHEST 1V PORT SAME DAY', # Portable same day
        'DG CHEST PORTABLE',     # Generic portable
        'XR chest 1V',           # X-ray single view
        'Chest',                 # Generic chest
        'CHEST'                  # Generic chest
    ]
    ...
    for i, filename in enumerate(sample_files):
        ...
        try:
            ...
            study_descriptions_found.add(study_desc)
            ...
            if '2V' in study_desc:
                ...
                view_type = "PA"  # Generic chest studies are usually PA
    ...
    print("="*60)
    print(f"Total sample files processed: {len(sample_files)}")
    print(f"Frontal views extracted: {extracted_count}")
    print(f"Skipped files (not frontal): {skipped_count}")
    print(f"Skipped files (no images): {no_images_count}")
    print(f"Extraction rate: {(extracted_count/len(sample_files))*100:.1f}%")
    
    print(f"\nStudy descriptions found:")
    for desc in sorted(study_descriptions_found):
        print(f"  - {desc}")
    ...
    if csv_data:
        ...
        for entry in csv_data:
            ...
            study_desc = entry['study_description']
            view_types[view_type] = view_types.get(view_type, 0) + 1
            study_descriptions[study_desc] = study_descriptions.get(study_desc, 0) + 1
        ...
        for desc, count in sorted(study_descriptions.items(), key=lambda x: x[1], reverse=True)[:5]:
            print(f"  {desc}: {count} ({count/len(csv_data)*100:.1f}%)")
...
def find_study_images(study_id, images_dir):
    ...
    # Remove 'p' prefix from patient ID to match image directory names
    if patient_id.startswith('p'):
        patient_id = patient_id[1:]  # Remove 'p' prefix
